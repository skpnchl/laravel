<!DOCTYPE html>
<html lang="en">
	<head>
		<meta charset="UTF-8">
		<title>SKPWEBSITE - Contact</title>
		<meta http-equiv="X-UA-Compatible" content="IE=edge">
		<meta name="viewport" content="width=device-width, initial-scale=1">
		<link rel="icon" type="image/png" href="../favicon.png">
		<link rel="canonical" href="http://www.skpwebsite.com">
		<meta name="description" content="">
		<link rel="stylesheet" href="{{ URL::asset('css/app.css') }}">
	</head>
	<body>
		
		<div class="login-m-m">
			<div class="logo-m-m"><a href="{{ route('home') }}"><img src="data:image/svg+xml;base64," alt="skpwebsite"/></a>
			<a href="{{ route('home') }}">
				<div class="cross-log">
					<span> </span>
					<span> </span>
				</div>
			</a></div>
			<div class="m-m1 contact-us-s">
				<h1 class="en1">contact me</h1>
				<p class="mssg animated bounceIn" style="color: green; display: none;"></p>
				<p class="mssgerrer" style="color: red; display: none;"></p>
				<form action="#" style="text-align: left;" id="sendcontact">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">
					<input type="text" name="name" id="ask-name" required="" class="fullname inco"/>
					<label for="name" class="fullname-l laco"><span>FULL NAME</span></label>
					<input type="email" name="email" id="ask-email" required="" class="email inco"/>
					<label for="email" class="email-l laco"><span>EMAIL</span></label>
					<br>
					<div class="textareawrap">
					<label for="massage"><p class="ask-text-lab">Type your query</p></label>
						<textarea name="massage" id="ask-massage" cols="2" rows="6" 
						onclick="this.focus();this.select()"
						onfocus="this.select()">What do you need...?</textarea>
					</div>
					<input type="submit" name="submit" id="ask-submit" value="SEND" class="submit s1-item2"/>
				</form>
			</div>
		</div>

		{{-- this is loading bar --}}

		<div class="loading" style="display: none"><div></div></div>

	<script src="{{ URL::asset('js/app.js') }}"></script>
	<script>
	    function addingC(sele, addclass) {
	        $(sele + ':eq(1)').addClass(addclass)
	    }
	    function fullappere (o,r,t){
	        var i=$(window).height(),
	        n=$(window).scrollTop(),
	        s=$(o),e=s.offset().top,
	        l=s.height(),
	        a=n+i>=e,
	        f=!(n-l>=e),
	        w=a&&f;
	        if(!w)return!1;
	        var p=-($(window).scrollTop()/r-t);
	        $(o).css({transform:"translateY("+p+"%)"})
	    };

	    

	    $('#sendcontact').submit(function (e) {
	    	e.preventDefault();
	    	$('.loading').show();
	    	var name = $('#ask-name').val(),
				email= $('#ask-email').val(),
				massage = $('#ask-massage').val();
			$.ajax({
				type: 'post',
				url: 'contactajax',
				data: {name: name,email: email,massage: massage},
				success: function(data){
					$('.mssgerrer').hide();
					$('.mssg').show();
					$('.mssg').html(' Hello <b style="font-size:18px">'+data+ '</b> your query succesfully sent.')
	    			$('#ask-name').val('');
					$('#ask-email').val('');
					$('#ask-massage').val('');
					$('.loading').hide();
				},
				error: function (jqXHR, exception) {
					$('.mssg').hide();
					$('.mssgerrer').show();
			        var msg = 'Errer';
			        if (jqXHR.status === 0) {
			            msg = 'Not connect.\n Verify Network.';
			        } else if (jqXHR.status == 404) {
			            msg = 'Requested page not found. [404]';
			        } else if (jqXHR.status == 500) {
			            msg = 'Internal Server Error [500].';
			        } else if (exception === 'parsererror') {
			            msg = 'Requested JSON parse failed.';
			        } else if (exception === 'timeout') {
			            msg = 'Time out error.';
			        } else if (exception === 'abort') {
			            msg = 'Ajax request aborted.';
			        } else {
			            msg = 'Uncaught Error.\n' + jqXHR.responseText;
			        }
			        $('.mssgerrer').html(msg);
			        $('.loading').hide();
			    }
			});
	    })


	</script>
</body>
</html>